...
def test_set_price_feed_contract():
    ...
    # Assert
    # checking that the address of the price feed mapping is the same as the price feed address set
...
def test_stake_tokens(amount_staked):
    ...
    )  # this is approving the token farm to utilize the amount staked from our accounts
    token_farm.stakeTokens(
        amount_staked, dapp_token.address, {"from": account}
    )  # this is staking the amount from our account
    # Assert
    assert (
        token_farm.stakingBalance(dapp_token.address, account.address)
        == amount_staked  # this is checking the stakers balance is equal to the amount staked
    )
    assert (
        token_farm.uniqueTokensStaked(account.address) == 1
    )  # this is checking that only one unique instance of the token is available currently
    assert (
        token_farm.stakers(0) == account.address
    )  # this is checking that the staker at 0 index is currently the same as the account that called the stake token function
